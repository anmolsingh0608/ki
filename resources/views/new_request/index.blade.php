...
@section('title', 'New Card Requests')
    ...
    <div class="container-xxl px-xxl-0">
        <div class="align-items-center d-flex mb-2 row">
            <div class="align-items-center col-md-6 d-flex">
                <h2 class="m-0 mb-0 title">New Membership Card Requests </h2>
            </div>
            <div class="col-md-6">
                <form class="align-items-center d-flex form-ki justify-content-end" action="{{ route('newrequest.index') }}" method="GET">
        ...
        <p>All ({{count($new_requests)}})</p>
        <p>Newly added membership card requests. To process, select an order and export a CSV for card printing.</p>
        ...
        <div class="row mb-3">
            <div class="col-sm-3 pe-md-0 bAction">
                ...
                <div class="dropdown-menu dropdown-menu-1">
                    <a id="bulk-delete" class="dropdown-item" data-uri="{{ route('newrequest.deleteall') }}">Delete Selected</a>
        ...
        <table class="table table-bordered table-striped table-hover">
            <tbody>
                ...
                <tr>
                    <td scope="row"><input type="checkbox" class="singlechkbox" data-id="{{$new_request->id}}" /> <a href="{{ route('membership.index', $new_request->id) }}" class="anchor">{{ $new_request->id }}</a></td>
                    <td scope="row">{{ date_format($new_request->created_at,'d M Y') }}</td>
                    <td scope="row"><a href="{{ route('membership.index', $new_request->id) }}" class="anchor">{{ $new_request->name }}</a></td>
                    <td scope="row">{{ $new_request->no_of_cards }}</td>
                    <td scope="row"><a href="{{ route('newrequest.invoice_to', $new_request->invoice_to) }}" class="anchor">{{ $new_request->invoice_to }}</a></td>
                    <td scope="row"><a href="{{ route('newrequest.ship_to', $new_request->ship_to) }}" class="anchor">{{ $new_request->ship_to }}</a></td>
                    <td scope="row" class="d-flex">

                        <a href="{{ route('newrequest.edit',$new_request->id) }}" class="btn" style="padding: 0 .50rem;">
                        ...
                        <form></form>
                        <form action="{{ route('newrequest.destroy', $new_request->id) }}" method="post">
                            @csrf
                            @method('DELETE')
                            <button class="btn" type="submit" onclick="return confirm('Do you want to delete this request?')" style="padding: 0 .50rem;">
